...
/**
 * Document entity editor screen which using special for workflow extension
 *
 * @author adiatullin
 */
public class QueryWorkflowEdit extends AbstractEditor<Document> {
    ...
    @Inject
    protected DatasourceImplementation<Document> documentDs;
    ...
    private Employee getInitiator() {//get possible initiator by current user
        List<Employee> list = dataManager.loadList(LoadContext.create(Employee.class)
                .setQuery(new LoadContext.Query("select e from cifra$Employee e where e.user.id = :userId")
                        .setParameter("userId", userSessionSource.getUserSession().getUser().getId())
                        .setMaxResults(1))
